...
class UsBankHomePage:
	...
	def IsValidLoginPage(self):
		...
		if(len(userNameTag) == 0):
			print('Login page does NOT have uname element')
			return False
		print('Login page is valid')
		return True

	def _checkForAuthenticationFailedError(self):
		print('Checking for authentication failed error')
	...
	def Login(self, username, password):
		print('Login in using %s' % (self._browser.current_url))		
		self._checkForAuthenticationFailedError()
		print('Has Auth Error %s' % (self._authenticationFailedError))
		if (self._authenticationFailedError == True):
			print('Page has authentication error')
			return False

		print('login to page with username %s and password %s' % (username, password))
